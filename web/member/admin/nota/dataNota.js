...
export default function dat(){
    ...
    async function DeletUser(id_nota){
  
        var res = await fetch(`/api/nota/delnota?nim=${id_nota}`, { method : 'DELETE' })
                ...
                <tr key ={idx}>
                    ...
                    <div className='d-flex justify-content-between'>
                        <Link href={`/admin/nota/updatenota?id_nota=${usr.id_nota}&id_user=${usr.id_user}&id_barang=${usr.id_barang}&harga=${usr.harga}&order_by=${usr.order_by}`}>
                            ...
                            <td>
                            <button type='button' value={usr.id_nota} onClick={(e)=>DeletUser(e.target.value)}>Delete</button>
                            </td>
